...
def query_prepare1(expid, cleaned_task_data, padding_id, num_queries, detour_rate=0.1):
    ...
    for i in range(num_queries):
        ...
        detour_pos = np.random.choice(
            row["path_len"], int(row["path_len"] * detour_rate), replace=False
        )  # 随机选几个位置
        path = [
            detour(0) if i in detour_pos else e for i, e in enumerate(row["cpath_list"])
        ]
        q_arr[i, : row["path_len"]] = np.array(path, dtype=np.int32)

    y = random_index[:num_queries]
    query_route_length = cleaned_task_data["path_len"].values[
        random_index[:num_queries]
    ]
...
# 随机选位置，detour替换，垃圾版
def query_prepare2(
    expid, cleaned_task_data, padding_id, num_queries, indexes, detour_rate=0.1
):
    min_len, max_len = 10, 100
    num_samples = len(cleaned_task_data)

    def detour(
        rate, road_id, indexes
    ):  # 有1-rate的概率替换一个新的路段，原始设置0.9，基本就没换
        p = np.random.random_sample()  # 产生[0,1)之间的随机数
        return (
            indexes[road_id][np.random.randint(indexes.shape[1])]
            if p > rate
            else padding_id
        )
    ...
    for i in range(num_queries):
        ...
        path = [
            detour(0, e, indexes) if i in detour_pos else e
            for i, e in enumerate(row["cpath_list"])
        ]
        q_arr[i, : row["path_len"]] = np.array(path, dtype=np.int32)
...
def query_prepare3(
    expid, cleaned_task_data, padding_id, num_queries, detour_base, detour_rate=0.1
):
    np.random.seed(expid)
    min_len, max_len = 10, 100
    num_samples = len(cleaned_task_data)
    ...
    for i in range(num_queries):
        ...
        detour_pos = np.random.choice(
            sample_pos, sample_num, replace=False
        )  # 随机选几个位置
...
def query_prepare4(
    ...
    feature_df["road_speed"][feature_df["road_speed"] < 1] = feature_df[
        "road_speed"
    ].mean()
    feature_df["road_travel_time"] = (
        feature_df["length"] / feature_df["road_speed"]
    ).astype(int)
    ...
    def detour(rate, path, tm_list, detour_pos, history_travel_time_dict):
        ...
        for road in new_path:
            ...
            if road in cur_travel_time_dict.keys():
                tm_list.append(
                    tm_list[-1] + cur_travel_time_dict[road] * (1 + shift_ratio)
                )
            else:
                tm_list.append(
                    tm_list[-1] + history_travel_time_dict[road] * (1 + shift_ratio)
                )
    ...
    tm_arr = torch.cat(
        [
            ...
            torch.zeros_like(torch.LongTensor(minute_arr).unsqueeze(dim=2)).long(),
        ],
        dim=-1,
    )
...
def query_prepare5(
    ...
    def dfs_path(
        ...
        while stack:
            ...
            if (
                vertex == end and path != detour_path
            ):  # 如果到达终点，则将路径添加到结果列表中
                paths.append(path)
            ...
            for neighbor in (
                torch.nonzero(trans_mat[vertex] != 0)
                .reshape(
                    -1,
                )
                .numpy()
    ...
    for i in tqdm(range(num_queries)):
        ...
        start_pos = np.random.randint(
            1, row["route_length"] - sample_len
        )  # OD不参与处理
        detour_path = row["cpath_list"][start_pos : start_pos + sample_len]
        detour_travel_time = (
            row["road_timestamp"][start_pos + sample_len - 1]
            - row["road_timestamp"][start_pos]
        )
        path, tm_list = detour(
            0,
            row["cpath_list"],
...
def query_prepare5(
        ...
        while stack:
            ...
            if (
                ...
                for road in poly:
                    pt_list += [
                        float(item)
                        for pair in geometry_df.iloc[road]["geometry"][12:-1].split(
                            ", "
                        )
                        for item in pair.split(" ")
                    ]
                area = Polygon(
                    [[pt_list[i], pt_list[i + 1]] for i in range(0, len(pt_list), 2)]
                ).area
            ...
            if (
                len(path) - 1 == int(1 / 3 * len(origin_path)) + 1
            ):  # 如果长度到达最大长度但不符合标准则去除
                continue
    ...
    for i in tqdm(range(num_queries)):
        ...
        while (
            np.array_equal(path, row["cpath_list"])
            and try_count < 10
            and len(sample_pos_list) != 0
        ):
            try_count += 1
            ...
            path, tm_list = detour(
                0, row["cpath_list"], row["road_timestamp"], detour_path, start_pos
            )
...
def evaluation(
    ...
    for expid in range(fold):
        queries, y, query_route_length = query_prepare3(
            expid, cleaned_split_df, num_nodes, num_queries, detour_base, detour_rate
        )
        ...
        if road_embedding is None:
            ...
            route_data = (
                torch.zeros((route_assign_mat.shape[0], route_assign_mat.shape[1], 2))
                .long()
                .cuda()
            )
            queries = (
            ...
            q = get_seq_emb_from_node(
                road_embedding, queries, query_route_length, batch_size
            )
        ...
        print(
            f"exp {expid} | Mean Rank: {rank_sum / num_queries:.4f}, HR@10: {hit / (num_queries - no_hit):.4f}, No Hit: {no_hit}"
        )

    print(
        f"similarity search       | Mean Rank: {np.mean(mean_rank_list):.4f}, HR@10: {np.mean(hit_list):.4f}, No Hit: {np.mean(no_hit_list)}"
    )
...
def evaluation2(
    ...
    hit_list, mean_rank_list, no_hit_list = [], [], []
    for expid in range(fold):
        queries, y, query_route_length, tm_mat = query_prepare4(
            expid,
            cleaned_split_df,
            num_nodes,
            num_queries,
...
def evaluation3(
    ...
    hit_list, mean_rank_list, no_hit_list = [], [], []
    for expid in range(fold):
        queries, y, query_route_length, tm_mat = query_prepare5(
            expid,
            cleaned_split_df,
            num_nodes,
            num_queries,
        ...
        if road_embedding is None:
            ...
            q = get_seq_emb_from_node(
                road_embedding, queries, query_route_length, batch_size=1024
            )
...
def finetune_evaluation2(
    ...
    detour_rep = get_seq_emb_from_traj_withRouteOnly(
        seq_model, queries, batch_size=1024
    ).cuda()

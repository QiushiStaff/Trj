...
def prepare_route_data(df, mat_padding_value, data_padding_value, max_len):
    ...
    weekday_route_list = [
        torch.tensor(weekday[:-1]).long() for weekday in weekday_route_list
    ]  # road_timestamp 比 route 本身的长度多1，包含结束的时间戳
    minute_route_list = [
        torch.tensor(minute[:-1]).long() for minute in minute_route_list
    ]  # road_timestamp 比 route 本身的长度多1，包含结束的时间戳
...
class StaticDataset(Dataset):
    def __init__(
        self, data, mat_padding_value, data_padding_value, gps_max_len, route_max_len
    ):
        ...
        gps_length = (
            data["opath_list"]
            .apply(
                lambda opath_list: self._split_duplicate_subseq(
                    opath_list, data["route_length"].max()
                )
            )
            .tolist()
        )
        ...
        self.gps_data = (
            gps_data  # gps point shape = (num_samples,gps_max_length,num_features)
        )
        ...
        route_data, route_assign_mat = prepare_route_data(
            data[["cpath_list", "road_timestamp", "road_interval"]],
            mat_padding_value,
            data_padding_value,
            route_max_len,
        )
        ...
        self.route_assign_mat = (
            route_assign_mat  # shape = (num_samples,route_max_length)
        )
    ...
    def __getitem__(self, idx):
        return (
            self.gps_data[idx],
            self.gps_assign_mat[idx],
            self.route_data[idx],
            self.route_assign_mat[idx],
            self.gps_length[idx],
        )
...
def get_loader(
    ...
    train_data, test_data, val_data = (
        dataset[dataset["flag"] < 14],
        dataset[dataset["flag"] == 14],
        dataset[dataset["flag"] == 15],
    )
    ...
    train_dataset = StaticDataset(
        train_data, mat_padding_value, data_padding_value, gps_max_len, route_max_len
    )
    test_dataset = StaticDataset(
        test_data, mat_padding_value, data_padding_value, gps_max_len, route_max_len
    )
    val_dataset = StaticDataset(
        val_data, mat_padding_value, data_padding_value, gps_max_len, route_max_len
    )

    train_loader = DataLoader(
        train_dataset, batch_size=batch_size, drop_last=True, num_workers=num_worker
    )
    val_loader = DataLoader(
        val_dataset, batch_size=batch_size, drop_last=True, num_workers=num_worker
    )
    test_loader = DataLoader(
        test_dataset, batch_size=batch_size, drop_last=True, num_workers=num_worker
    )

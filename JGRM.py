...
class JGRMModel(BaseModel):
    def __init__(
        ...
        self.route_padding_vec = torch.zeros(
            1, road_embed_size, requires_grad=True
        ).cuda()
        ...
        self.route_encoder = TransformerModel(
            hidden_size, 8, hidden_size, 4, drop_route_rate
        )

        # gps encoding
        self.gps_linear = nn.Linear(gps_feat_num, gps_embed_size)
        self.gps_intra_encoder = nn.GRU(
            gps_embed_size, gps_embed_size, bidirectional=True, batch_first=True
        )  # 路段内建模
        ...
        self.sharedtransformer = TransformerModel(
            hidden_size, 4, hidden_size, 2, drop_road_rate
        )
    ...
    # concat gps and route representation, then use shared transformer to update
    def encode_joint(
        self,
        route_road_rep,
        route_traj_rep,
        ...
        # based on class of representation, to find a unique vector that can be learn.
    ...
    def forward(
        ...
        # route_traj_rep,(batch,), based on mean pooling of each route representation
